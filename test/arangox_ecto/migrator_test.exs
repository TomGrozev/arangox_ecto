defmodule ArangoXEcto.MigratorTest do
  use ExUnit.Case
  ...
  defmodule ChangeMigrationPrefix do
    ...
    def change do
      create collection(:comments, prefix: "foo") do
        add :name, :string
      end
  ...
  test "execute one anonymous function" do
    ...
    message = "no function clause matching in ArangoXEcto.Migration.Runner.command/1"
  ...
  defp create_migration(name, opts \\ []) do
    ...
    File.write!(name, """
    defmodule ArangoXEcto.MigrationTest.S#{module} do
      use ArangoXEcto.Migration
  ...
  test "logs migrations" do
    ...
    assert output =~ "== Running 12 ArangoXEcto.MigratorTest.UpDownMigration.down/0 forward"
  ...
  test "up raises error when missing up/0 and change/0" do
    assert_raise Ecto.MigrationError, fn ->
      up(TestRepo, 10, InvalidMigration, log: false)
    end
  end

  test "down raises error when missing down/0 and change/0" do
    assert_raise Ecto.MigrationError, fn ->
      down(TestRepo, 1, InvalidMigration, log: false)
    end
  end
  ...
  describe "migration options" do
    ...
    test "skip _migrations collection creation" do
      in_tmp(fn path ->
        create_migration("15_sample.exs")

        expected_result = [{:up, 15, "sample"}]
        assert migrations(TestRepo, path, skip_collection_creation: true) == expected_result

        # assert opts[:skip_collection_creation] == true

        assert match?(nil, last_command())
      end)
    end

    test "default to create _migrations collection" do
      in_tmp(fn path ->
        create_migration("15_sample.exs")
        ...
        assert match?({:create_if_not_exists, %_{name: "_migrations"}, _}, last_command())
      end)
    end
  end
  ...
  describe "run" do
    ...
    test "fails if there is no migration in file" do
      in_tmp(fn path ->
        ...
        assert_raise Ecto.MigrationError,
                     "file 13_sample.exs is not an ArangoXEcto.Migration",
                     fn ->
    ...
    test "raises if target is not integer" do
      in_tmp(fn path ->
        ...
        assert_raise(
          FunctionClauseError,
          message_base <> "ArangoXEcto.Migrator.pending_to/4",
          fn ->
        ...
        assert_raise(
          FunctionClauseError,
          message_base <> "ArangoXEcto.Migrator.pending_to_exclusive/4",
          fn ->
  ...
  describe "alternate migration source format" do
    test "fails if there is no migration in file" do
      assert_raise Ecto.MigrationError,
                   "module EmptyModule is not an ArangoXEcto.Migration",
                   fn ->
    ...
    test "fails if the module does not define migrations" do
      assert_raise Ecto.MigrationError,
                   "ArangoXEcto.MigratorTest.InvalidMigration does not implement a `up/0` or `change/0` function",
                   fn ->
    ...
    test "fails if there are duplicated name" do
      assert_raise Ecto.MigrationError,
                   "migrations can't be executed, migration name Elixir.ArangoXEcto.MigratorTest.ChangeMigration is duplicated",
                   fn ->
  ...
  describe "gen_server" do
    test "runs the migrator with repos config" do
      ...
      assert {:ok, :undefined} =
               start_supervised({ArangoXEcto.Migrator, [repos: [TestRepo], migrator: migrator]})
    end
    ...
    test "skip is set" do
      ...
      assert {:ok, :undefined} =
               start_supervised(
                 {ArangoXEcto.Migrator, [repos: [TestRepo], migrator: migrator, skip: true]}
               )
    end
